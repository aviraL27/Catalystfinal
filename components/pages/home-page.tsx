...
import LatestEventsModal from "../latest-events-modal"
...
export default function HomePage({ onNavigate, onRaiseIssue }: HomePageProps) {
  ...
  useEffect(() => {
    const fetchIssues = async () => {
      try {
        const userId = localStorage.getItem("studentBtId") || ""
        const response = await fetch(`/api/issues?userId=${encodeURIComponent(userId)}`)
  ...
  return (
    <>
      <section className="page active">
        ...
        <div
          onClick={() => window.open("https://clubs-hub-drab.vercel.app/", "_blank")}
          style={{
            marginTop: "24px",
            padding: "24px",
            borderRadius: "12px",
            background: "linear-gradient(135deg, rgba(108, 92, 231, 0.15) 0%, rgba(45, 156, 219, 0.15) 100%)",
          ...
          onMouseEnter={(e) => {
            ...
            e.currentTarget.style.boxShadow = "0 20px 60px rgba(108, 92, 231, 0.3)"
        ...
        >
          <div style={{ position: "relative", zIndex: 1 }}>
            <h2 style={{ margin: "0 0 8px 0", fontSize: "24px", fontWeight: "700" }}>🎭 Know Your Clubs</h2>
            <p style={{ margin: "0", color: "rgba(255, 255, 255, 0.7)", fontSize: "14px" }}>
              Explore all the amazing clubs at IIIT Nagpur and find your community
            </p>
          </div>
          <div
            style={{
      ...
      <LatestEventsModal isOpen={showLatestModal} onClose={() => setShowLatestModal(false)} />
    </>
  )
}
