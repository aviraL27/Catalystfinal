...
export default function AdminDashboard() {
  ...
  const filteredIssues =
    filter === "all"
      ...
      : issues.filter((issue) => {
          if (filter === "critical") {
            return issue.urgency.toLowerCase() === "critical" || issue.category.toLowerCase() === "medical"
          }
          return issue.urgency.toLowerCase() === filter.toLowerCase()
        })

  const sortedIssues = [...filteredIssues].sort((a, b) => {
    const getPriority = (issue: Issue) => {
      if (issue.category.toLowerCase() === "medical") return 0
      if (issue.urgency.toLowerCase() === "critical") return 1
      if (issue.urgency.toLowerCase() === "high") return 2
      if (issue.urgency.toLowerCase() === "medium") return 3
      if (issue.urgency.toLowerCase() === "low") return 4
      return 5
    }
  ...
  const getUrgencyColor = (urgency: string, category: string) => {
    if (category.toLowerCase() === "medical") {
      return "destructive"
    }
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2 flex-wrap">
                            ...
                            <Badge variant={getUrgencyColor(issue.urgency, issue.category) as any}>
                              {issue.category.toLowerCase() === "medical"
                                ? "CRITICAL (MEDICAL)"
